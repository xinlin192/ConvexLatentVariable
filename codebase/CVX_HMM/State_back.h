...
class PairHMMState:public State{
	...
	virtual vector<pair<State*,double> > transProb(vector<char>& x, vector<char>& ref_seq){
		...
		if( s_type==BEGIN ){
			...
			next_states.push_back(make_pair( new PairHMMState(I,-1,0), ins_prob ));
			...
			next_states.push_back(make_pair( new PairHMMState(I,s_i,r_i), del_prob_del ));
		}
