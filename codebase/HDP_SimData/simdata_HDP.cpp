...
int main(int argc, char** argv){
	...
	if( argc < 7 ){
		cerr << "./simdata_HDP [num_doc] [doc_len] [alpha_doc_topic] [alpha_topic] [alpha_topic_word] [alpha_word]" << endl;
		exit(0);
	}
	
	int num_doc = atoi(argv[1]);
	int num_word_p_doc = atoi(argv[2]);
	int alpha_doc_topic = atoi(argv[3]);
	int alpha_topic = atoi(argv[4]);
	int alpha_topic_word = atoi(argv[5]);
	int alpha_word = atoi(argv[6]);
	
	vector<FreqList> topic_word_freq;
	vector<int> topic_word_sum;
	
	vector<FreqList> doc_topic_freq;
	vector<int> doc_topic_sum;
	...
	for(int i=0;i<num_doc;i++){
		...
		doc_topic_freq.push_back(FreqList()) ;
		doc_topic_sum.push_back(0);
		
		for(int j=0;j<num_word_p_doc;j++){
			
			//Sample topic according to Hierachical DP
			topic = sample(doc_topic_freq[i], doc_topic_sum[i], alpha_doc_topic);
			if( topic >= doc_topic_freq[i].size() ){
			...
			addToFreq(topic, doc_topic_freq[i], doc_topic_sum[i]);
	...
	////doc topic freq
	fout.open("doc_topic_freq");
	for(int i=0;i<doc_topic_freq.size();i++){
		fout << i << " ";
		for(int j=0;j<doc_topic_freq[i].size();j++){
			if( doc_topic_freq[i][j] != 0)
				fout << j << ":" << doc_topic_freq[i][j]  << " ";
		}
		fout << endl;
	}
	fout.close();
